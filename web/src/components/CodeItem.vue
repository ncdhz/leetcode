<template>
  <a-card hoverable style="margin-bottom: 20px;">
    <template #cover>
      <div style="margin: 0px 2%; width:96%;" @click="openCode">
        <code-content :type="type" :code="getShowCode(getCodes(($db?.code as string[])[props.data[0]]))"/>
      </div>
    </template>
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['openCode'])
  function openCode() {
    emit('openCode')
  }
